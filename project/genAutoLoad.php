...
$cur_path = dirname(dirname(__FILE__));
$src_path = $cur_path . DIRECTORY_SEPARATOR . "src";
$config = array(
    'AUTOLOAD_PATH' => $src_path,
    'GEN_FILE' => $src_path.DIRECTORY_SEPARATOR."auto_load.php",
    "CACHE_KEY" => "sogou:case:genautoload"
);
try{
    $a = new AssemblyBuilder();
    $a->run($config['AUTOLOAD_PATH'],$config['GEN_FILE'],$config['CACHE_KEY']);
}catch (Exception $e){
...
{/*{{{*/
    ...
    private static $_skipFolders = array('web-inf', 'tmp', '.svn', 'sqls', 'logs', 'project','.git');
    private static $_skipFiles= array();
    private static $_fileNameTemplate = array('php');
    private static $_webRootDir = 'w4';//web根目录名
...
?>';
	 ...
	 * 在jiayuan的框架中需要。
	 * */
    static private function changePathNameForjy($file_name){
        if(DIRECTORY_SEPARATOR=='\\'){
            $file_name = str_replace('\\','/',$file_name);
        }
        preg_match("/\/(.*)\/".self::$_webRootDir."\/(.*)/",$file_name,$match_arr);
        if(count($match_arr)==3)
        {
            $filename = "WWW_ROOT_PATH.'".$match_arr[2]."'";
        }
        return $filename;
    }
